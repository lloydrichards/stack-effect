...
Stack Effect resolves those dependencies into a **Blueprint**. Because the HTTP API needs `domain-api-contracts`, the Blueprint adds the domain target and that module. This happens in memory: no repository files have been read or written.
