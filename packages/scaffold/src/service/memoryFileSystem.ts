...
const { badArgument, systemError } = PlatformError;
type PlatformError = PlatformError.PlatformError;
type SystemErrorTag = PlatformError.SystemErrorTag;
...
interface WatchSubscription {
  ...
  queue:
    | Queue.Enqueue<FileSystem.WatchEvent, PlatformError | Cause.Done>
    | undefined;
...
const includesWatchPath = (
  ...
  (recursive &&
    (watchedPath === "/" || eventPath.startsWith(`${watchedPath}/`))) ||
...
const attachDirectory = Effect.fnUntraced(function* (
  ...
  const expectedUnlinkedLinks =
    entry._tag === "Directory"
      ? DIR_SELF_LINK_COUNT +
        [...HashMap.values(entry.entries)].filter(
          (child) => findInode(state, child)?._tag === "Directory",
        ).length
      : 0;
  if (
    entry._tag !== "Directory" ||
    parent === inode ||
    entry.nlink !== expectedUnlinkedLinks
  ) {
    return yield* permissionDenied(
      method,
      name,
      "Cannot attach this directory",
    );
  }
  const now = yield* DateTime.now;
  const nextParent = {
...
const resolve = Effect.fnUntraced(function* (
  ...
  const currentInode = () => stack.at(-1) ?? RootInode;
  ...
  while (components.length > 0) {
    ...
    if (component.length === 0) {
      if (components.length === 0) {
        yield* getDirectory(state, currentInode(), method, originalPath);
      }
      continue;
    }
    if (component === ".") {
    ...
    const parent = yield* getDirectory(
      state,
      currentInode(),
      method,
      originalPath,
...
const getOpenFile = (
  ...
  Effect.suspend(() => {
    const descriptor = Option.getOrUndefined(
      HashMap.get(state.descriptors, fd),
    );
    if (descriptor === undefined) {
      return Effect.fail(
        descriptorError(fd, method, "File descriptor is closed"),
      );
    }
    if (access !== undefined && !descriptor[access]) {
      return Effect.fail(
        descriptorError(fd, method, `File descriptor is not ${access}`),
      );
    }
    const entry = findInode(state, descriptor.inode);
    if (entry === undefined || entry._tag !== "File") {
      return Effect.fail(
        descriptorError(fd, method, "File descriptor does not refer to a file"),
      );
    }
    return Effect.succeed([descriptor, entry]);
  });

const withSystemErrorPath = (
  error: PlatformError,
  method: string,
  path: string,
): PlatformError =>
...
const validateMode = (method: string, mode: number | undefined) =>
  mode === undefined ||
  (Number.isInteger(mode) && mode >= 0 && mode <= 0xffff_ffff)
    ? Effect.void
    : Effect.fail(
        argumentError(method, "mode must be an unsigned 32-bit integer"),
      );
...
const detachEntry: (
  ...
  method: string,
  path: string,
) => Effect.Effect<State, PlatformError> = Effect.fnUntraced(
  function* (state, target, now, recursive, method, path) {
    if (
      target.entry._tag === "Directory" &&
      HashMap.size(target.entry.entries) > 0 &&
      !recursive
    ) {
    ...
    if (target.entry._tag === "Directory") {
      const children = [...target.entry.entries].sort(([left], [right]) =>
      ...
      Effect.gen(function* () {
        ...
        for (let index = 0; index < pieces.length; index++) {
          ...
          const existing = yield* Effect.result(
            resolve(nextState, candidate, { method }),
          );
          ...
          nextState = yield* attachDirectory(
            nextState,
            parent.inode,
            parent.name,
            inode,
    ...
    return yield* volume.mutate((state) =>
      Effect.gen(function* () {
        const source = yield* resolve(state, fromPath, { method }).pipe(
          Effect.mapError((error) =>
            withSystemErrorPath(error, method, fromPath),
          ),
        );
        const destination = yield* resolveParent(state, toPath, method);
        const nextState = yield* linkInode(
          state,
          destination.inode,
          destination.name,
          source.inode,
          ...
          Effect.mapError((error) =>
            withSystemErrorPath(error, method, toPath),
          ),
        );
      ...
      Effect.gen(function* () {
        ...
        if (target.includes("\0")) {
          return yield* argumentError(
            method,
            "target must not contain a null byte",
          );
        }
...
const realPath = (volume: Volume) =>
  Effect.fnUntraced(function* (path: string) {
    return yield* volume.withState((state) =>
      Effect.map(resolve(state, path, { method: "realPath" }), (_) => _.path),
    );
  });
  ...
  Effect.fnUntraced(function* (
    ...
    return yield* volume.mutate((state) =>
      Effect.gen(function* () {
        ...
        return transitionResult(nextState, undefined, [
          { _tag: "Remove", path: target.success.path },
        ]);
      }),
    );
  });
...
const containsDirectory = (
  ...
  for (const inode of HashMap.values(entry.entries)) {
    if (
      findInode(state, inode)?._tag === "Directory" &&
      containsDirectory(state, inode, candidate)
    ) {
      return true;
    }
  }
    ...
    return yield* volume.mutate((state) =>
      Effect.gen(function* () {
        ...
        if (
          ...
          source.name === destinationParent.name
        ) {
          return transitionResult(state, undefined);
        }
        if (
          source.entry._tag === "Directory" &&
          containsDirectory(state, source.entry.ino, destinationParent.inode)
        ) {
          return yield* badResource(
            method,
            newPath,
        ...
        const destinationInode = findEntry(
          destinationParent.entry,
          destinationParent.name,
        );
        if (destinationInode === source.entry.ino)
          return transitionResult(state, undefined);
        ...
        if (destinationInode !== undefined) {
          ...
          if (
            source.entry._tag === "Directory" &&
            destination.entry._tag !== "Directory"
          ) {
            return yield* badResource(
              method,
              newPath,
          ...
          if (
            destination.entry._tag === "Directory" &&
            HashMap.size(destination.entry.entries) > 0
          ) {
            return yield* badResource(
              method,
              newPath,
        ...
        const sourceParent = yield* getDirectory(
          nextState,
          source.parent.ino,
          method,
          oldPath,
        );
        const moved = yield* getInode(
          nextState,
          source.entry.ino,
          method,
          oldPath,
        ...
        if (sourceParent.ino === destinationParent.inode) {
          ...
          const nextDestinationParent = yield* getDirectory(
            nextState,
            destinationParent.inode,
            method,
            newPath,
          );
          nextState = setInode(nextState, {
            ...
            nlink:
              moved._tag === "Directory"
                ? sourceParent.nlink - 1
                : sourceParent.nlink,
            mtime: now,
            ctime: now,
          });
          nextState = setInode(nextState, {
            ...
            entries: HashMap.set(
              nextDestinationParent.entries,
              destinationParent.name,
              moved.ino,
            ),
            nlink:
              moved._tag === "Directory"
                ? nextDestinationParent.nlink + 1
                : nextDestinationParent.nlink,
            mtime: now,
            ctime: now,
          });
        }
...
const cloneInode: (
  ...
  function* (state, source, context) {
    ...
    return yield* InodeEntry.$match(source, {
      ...
      SymbolicLink: (source) =>
        Effect.gen(function* () {
          const [createdState, inode] = yield* createSymbolicLink(
            state,
            source.target,
          );
          const nextState = applyMetadata(
      ...
      Directory: (source) =>
        Effect.gen(function* () {
          ...
          const children = [...source.entries].sort(([left], [right]) =>
            left.localeCompare(right),
          );
          for (const [name, childInode] of children) {
            const child = yield* getInode(nextState, childInode, method, path);
            const [clonedState, clone] = yield* cloneInode(
              nextState,
              child,
              context,
            );
          ...
          nextState = applyMetadata(
            nextState,
            yield* getDirectory(nextState, inode, method, path),
          );
          return [nextState, inode] as const;
        }),
    });
...
const validateCopyDirectoryContents: (
  ...
  function* (state, source, destination, overwrite, method, path) {
    for (const [name, sourceInode] of source.entries) {
      ...
      const destinationEntry = yield* getInode(
        state,
        destinationInode,
        method,
        path,
      ...
      if (
        sourceEntry._tag === "Directory" &&
        destinationEntry._tag === "Directory"
      ) {
        yield* validateCopyDirectoryContents(
          state,
          sourceEntry,
          destinationEntry,
          overwrite,
...
const copyDirectoryContents: (
  ...
  function* (state, source, destination, method, path, preserveTimestamps) {
    let nextState = state;
    const cloneContext = {
      method,
      sourcePath: path,
    ...
    for (const [name, sourceInode] of children) {
      ...
      const currentDestination = yield* getDirectory(
        nextState,
        destination.ino,
        method,
        path,
...
const resolveCopyFileDestination = Effect.fnUntraced(function* (
  ...
  while (true) {
    ...
    const resolved = yield* Effect.result(
      resolve(state, candidate, { method }),
    );
    ...
    candidate = symbolicLinkTargetPath(
...
const copyEntryUnlocked = Effect.fnUntraced(function* (
  ...
  if (
    source.entry._tag === "Directory" &&
    containsDirectory(state, source.inode, destination.inode)
  ) {
    return yield* badResource(
      method,
      toPath,
  ...
  if (source.entry._tag === "Directory" && existing !== undefined) {
    ...
    const nextState = yield* copyDirectoryContents(
      state,
      source.entry,
      existing,
      method,
...
const copyFile = (volume: Volume) =>
  Effect.fnUntraced(function* (fromPath: string, toPath: string) {
    return yield* volume.mutate((state) =>
      Effect.gen(function* () {
        const existing = yield* Effect.result(
          resolve(state, toPath, { method: "copyFile" }),
        );
        const [nextState, changed] = yield* copyFileUnlocked(
          state,
          fromPath,
          toPath,
        );
...
const openDescriptorUnlocked: (
  ...
  path: string,
  options?: OpenOptions,
) => Effect.Effect<readonly [State, OpenFileDescriptor], PlatformError> =
  Effect.fnUntraced(function* (state, path, options) {
    const flag = options?.flag ?? "r";
    if (!isOpenFlag(flag)) {
      return yield* argumentError(
        "open",
        "flag must be a supported file-open flag",
      );
    }
    ...
    while (entry === undefined) {
      ...
      if (unresolved._tag === "Success") {
        ...
        const resolved = yield* Effect.result(
          resolve(nextState, candidatePath, { method: "open" }),
        );
        if (resolved._tag === "Failure") {
          if (
            ...
            candidatePath = symbolicLinkTargetPath(
              unresolved.success.path,
              unresolved.success.entry.target,
            );
            continue;
      ...
      if (created._tag !== "File") {
        return yield* Effect.die(
          new Error("MemoryFileSystem.createFile produced a non-file inode"),
        );
      }
      if (options?.mode !== undefined) {
        created = {
      ...
      const linked = yield* Effect.result(
        linkInode(nextState, parent.inode, parent.name, inode, "open"),
      );
      ...
      if (linkedEntry._tag !== "File") {
        return yield* Effect.die(
          new Error("MemoryFileSystem.linkInode produced a non-file inode"),
        );
      }
      entry = linkedEntry;
    }
...
const openDescriptor: (
  ...
  function* (volume, path, options) {
    return yield* volume.mutate((state) =>
      Effect.gen(function* () {
        const existing = yield* Effect.result(
          resolve(state, path, { method: "open" }),
        );
        const [nextState, descriptor] = yield* openDescriptorUnlocked(
          state,
          path,
          options,
        );
        ...
        const tag =
          existing._tag === "Failure"
            ...
            : mode.truncate
              ...
              : undefined;
        if (tag === undefined)
          return transitionResult(nextState, descriptor.fd);
...
const closeDescriptor = Effect.fnUntraced(function* (
  ...
  return yield* volume.mutate((state) =>
    Effect.succeed(
      transitionResult(closeDescriptorUnlocked(state, fd), undefined),
    ),
  );
});
...
const writeDescriptorUnlocked = Effect.fnUntraced(function* (
  ...
  const position = descriptor.append
    ? entry.data.length
    : Number(descriptor.position);
  if (!Number.isSafeInteger(position) || position < 0) {
    return yield* descriptorError(fd, method, "Invalid file position");
  }
...
const writeDescriptor = (
  ...
  volume.mutate((state) =>
    Effect.gen(function* () {
      const [nextState, written] = yield* writeDescriptorUnlocked(
        state,
        fd,
        buffer,
        method,
      ...
      const descriptor = Option.getOrUndefined(
        HashMap.get(nextState.descriptors, fd),
      );
      return transitionResult(
        nextState,
        written,
...
class MemoryFile implements FileSystem.File {
  ...
  get stat(): Effect.Effect<FileSystem.File.Info, PlatformError> {
    return this.volume.withState((state) =>
      Effect.map(getOpenFile(state, this.fd, "stat"), ([, entry]) =>
        fileInfo(entry),
      ),
    );
  }
  ...
  get sync(): Effect.Effect<void, PlatformError> {
    return this.volume.withState((state) =>
      Effect.asVoid(getOpenFile(state, this.fd, "sync")),
    );
  }
  ...
  ): Effect.Effect<bigint, PlatformError> {
    const volume = this.volume;
    const fd = this.fd;
    return volume.mutate((state) =>
      Effect.gen(function* () {
        const descriptor = Option.getOrUndefined(
          HashMap.get(state.descriptors, fd),
        );
        if (descriptor === undefined) return transitionResult(state, 0n);
        const position =
          from === "start" ? offset : descriptor.position + offset;
        if (position < 0n) {
          return yield* descriptorError(fd, "seek", "Invalid file position");
        }
        return transitionResult(
          {
            ...state,
  ...
  readAlloc(
    ...
    if (!Number.isSafeInteger(length) || length < 0) {
      return Effect.fail(
        argumentError("readAlloc", "size must be a non-negative safe integer"),
      );
    }
          ...
          (bytesRead) =>
            bytesRead === 0
              ...
              : Option.some(
                  bytesRead === length
                    ? buffer
                    : buffer.slice(0, Number(bytesRead)),
                ),
        ),
    );
  }

  truncate(length: number = 0): Effect.Effect<void, PlatformError> {
    const volume = this.volume;
    const fd = this.fd;
    return Effect.flatMap(validateSize("truncate", length), (size) =>
      volume.mutate((state) =>
        Effect.gen(function* () {
          const [descriptor, entry] = yield* getOpenFile(
            state,
            fd,
            "truncate",
            "writable",
  ...
  writeAll(buffer: Uint8Array): Effect.Effect<void, PlatformError> {
    return Effect.asVoid(
      writeDescriptor(this.volume, this.fd, buffer, "writeAll"),
    );
  }
}

const open = (volume: Volume) => (path: string, options?: OpenOptions) =>
  Effect.acquireRelease(openDescriptor(volume, path, options), (fd) =>
    closeDescriptor(volume, fd),
  ).pipe(Effect.map((fd) => new MemoryFile(volume, fd)));
...
const access = (volume: Volume) =>
  Effect.fnUntraced(function* (path: string) {
    yield* volume.withState((state) =>
      Effect.asVoid(resolve(state, path, { method: "access" })),
    );
  });
  ...
  (
    ...
    volume
      .mutate((state) =>
        Effect.gen(function* () {
          const existing = yield* Effect.result(
            resolve(state, path, { method: "writeFile" }),
          );
          let [nextState, descriptor] = yield* openDescriptorUnlocked(
            state,
            path,
            {
              flag: options?.flag ?? "w",
          ...
          const [writtenState] = yield* writeDescriptorUnlocked(
            nextState,
            descriptor.fd,
            data,
            "writeAll",
          ...
          const resolved = yield* resolve(nextState, path, {
            method: "writeFile",
          });
          return transitionResult(
            nextState,
      ...
      .pipe(
        Effect.mapError((error) =>
          withOperationError(error, "writeFile", path),
        ),
      );
...
const validateSize = (method: string, size: number | undefined) => {
  ...
  return Number.isSafeInteger(value) && value >= 0
    ...
    : Effect.fail(
        argumentError(method, "size must be a non-negative safe integer"),
      );
};
...
const stat = (volume: Volume) =>
  Effect.fnUntraced(function* (path: string) {
    return yield* volume.withState((state) =>
      Effect.map(resolve(state, path, { method: "stat" }), ({ entry }) =>
        fileInfo(entry),
      ),
    );
  });
    ...
    return yield* volume.mutate((state) =>
      Effect.gen(function* () {
        ...
        const nextState = setInode(state, {
          ...resolved.entry,
          mode:
            (resolved.entry.mode & ~PERMISSION_MODE) | (mode & PERMISSION_MODE),
          ctime: now,
        });
        return transitionResult(
          nextState,
...
const validateOwner = (method: string, name: string, value: number) =>
  Number.isInteger(value) && value >= 0 && value <= 0xffff_ffff
    ? Effect.void
    : Effect.fail(
        argumentError(method, `${name} must be an unsigned 32-bit integer`),
      );
...
const dateTimeInput = (method: string, name: string, value: Date | number) => {
  const milliseconds =
    typeof value === "number" ? value * 1000 : value.getTime();
  if (!Number.isFinite(milliseconds)) {
    return Effect.fail(
      argumentError(
        method,
        `${name} must be a valid Date or epoch-seconds number`,
      ),
    );
  }
  ...
  return Option.isSome(parsed)
    ...
    : Effect.fail(
        argumentError(method, `${name} is outside the supported date range`),
      );
};
...
const allocateTempDirectory = Effect.fnUntraced(function* (
  ...
  while (true) {
    ...
    const currentParent = yield* getDirectory(
      nextState,
      parent.inode,
      method,
      parentPath,
...
const makeTempFileWithMethod = Effect.fnUntraced(function* (
  ...
  return yield* volume.mutateInterruptibly((state) =>
    Effect.gen(function* () {
      ...
      Effect.mapError((error) =>
        withOperationError(error, method, options?.directory ?? TEMP_DIR),
      ),
    ),
  );
});
  ...
  (options?: {
    ...
    Effect.acquireRelease(
      ...
      (path) => {
        ...
        return Effect.orDie(
          remove(volume)(directory, { recursive: true, force: true }),
        );
      },
    );
...
const expandBraces = (method: string, pattern: string) => {
  ...
  while (true) {
    const index = patterns.findIndex(
      (pattern) => findBraceExpansion(pattern) !== undefined,
    );
    ...
    if (
      patterns.length - 1 + expansion.alternatives.length >
      MAX_BRACE_EXPANSIONS
    ) {
      return Effect.fail(
        argumentError(
          method,
          `brace expansion exceeds ${MAX_BRACE_EXPANSIONS} alternatives`,
        ),
      );
    }
    patterns = [
...
const parseCharacterClass = (
    ...
    if (segment[index] === "\\") {
      ...
      if (index === segment.length) {
        return argumentError(
          method,
          "character classes must not end with an escape",
        );
      }
    }
    const value = segment[index];
    if (value === undefined) {
      return argumentError(method, "character classes must be closed");
    }
    characters.push({ value, escaped });
    index += 1;
  }
  if (index === segment.length || characters.length === 0) {
    return argumentError(
      method,
      "character classes must be closed and non-empty",
    );
  }
  ...
  for (
    ...
    if (
      ...
      if (character.value > end) {
        return argumentError(
          method,
          "character class ranges must be ascending",
        );
      }
  ...
  return Effect.succeed([
    GlobToken.CharacterClass({ negated, ranges, literals }),
    index + 1,
  ] as const);
};

const parseGlobSegment = Effect.fnUntraced(function* (
  method: string,
  segment: string,
) {
  ...
  while (index < segment.length) {
    ...
    if (character === "\\") {
      ...
      if (index === segment.length) {
        return yield* argumentError(
          method,
          "patterns must not end with an escape",
        );
      }
      const value = segment[index];
      if (value === undefined) {
...
const compileGlobPattern = Effect.fnUntraced(function* (
  method: string,
  pattern: string,
) {
  if (
    ...
    return yield* argumentError(
      method,
      "pattern must be a root-relative POSIX glob",
    );
  }
  ...
  if (
    ...
    return yield* argumentError(
      method,
      "pattern must not contain empty or dot path segments",
    );
  }
  const compiled = yield* Effect.forEach(segments, (segment) =>
    parseGlobSegment(method, segment),
  );
  return { segments: compiled, directoryOnly } satisfies CompiledGlobPattern;
});

const compileGlobPatterns = Effect.fnUntraced(function* (
  method: string,
  pattern: string,
) {
  const expanded = yield* expandBraces(method, pattern);
  return yield* Effect.forEach(expanded, (alternative) =>
    compileGlobPattern(method, alternative),
  );
});
...
const matchesGlobSegment = (pattern: GlobSegment, value: string): boolean => {
  ...
  while (valueIndex < value.length) {
    ...
    if (
      token !== undefined &&
      token._tag !== "Star" &&
      matchesGlobToken(token, value[valueIndex] ?? "")
    ) {
      patternIndex += 1;
      valueIndex += 1;
...
const matchesGlob = (
  ...
  let next = Array.from(
    ...
    (_, index) => index === path.length,
  );
  for (
    let patternIndex = pattern.segments.length - 1;
    patternIndex >= 0;
    patternIndex--
  ) {
    ...
    if (segment._tag === "Globstar") {
      for (let pathIndex = path.length; pathIndex >= 0; pathIndex--) {
        current[pathIndex] =
          (next[pathIndex] ?? false) ||
          (pathIndex < path.length &&
            !(path[pathIndex] ?? "").startsWith(".") &&
            (current[pathIndex + 1] ?? false));
      }
    } else {
      for (let pathIndex = path.length - 1; pathIndex >= 0; pathIndex--) {
        current[pathIndex] =
          matchesGlobSegment(segment, path[pathIndex] ?? "") &&
          (next[pathIndex + 1] ?? false);
      }
    }
    next = current;
  }
  ...
  Effect.fnUntraced(function* (
    ...
    return yield* volume.withState((state) =>
      Effect.gen(function* () {
        ...
        const rootExcluded = excludes.some((pattern) =>
          matchesGlob(pattern, [], true),
        );
        if (rootExcluded) return [];
        const matches: Array<string> = includes.some((pattern) =>
          matchesGlob(pattern, [], true),
        )
          ? ["."]
          : [];
        const pending: Array<readonly [DirectoryInode, ReadonlyArray<string>]> =
          [[resolved.entry, []]];
        while (pending.length > 0) {
          ...
          for (const name of [...HashMap.keys(directory.entries)].sort()) {
            ...
            const inode = findEntry(directory, name);
            const entry =
              inode === undefined ? undefined : findInode(state, inode);
            const isDirectory = entry?._tag === "Directory";
            const excluded = excludes.some((pattern) =>
              matchesGlob(pattern, path, isDirectory),
            );
            if (
              !excluded &&
...
const makeVolume = Effect.gen(function* () {
  ...
  const withState: Volume["withState"] = (use) =>
    lock.withPermit(Effect.suspend(() => use(state)));
  const mutate: Volume["mutate"] = (use) =>
    lock.withPermit(Effect.uninterruptible(commit(use)));
